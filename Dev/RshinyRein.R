...
##############################################################################################
  onclick("Run", {
    ...
    myclaims_comp <- append(myclaims_comp, "100")
    
    if(length(myclaims) != length(myclaims_comp)){
      print(length(myclaims))
      print(length(myclaims_comp))
      print("ERROR: Unable to parse all Company Codes.Please contact Henry Diep (x7209) for assistance.")
      js_string<-'alert("ERROR: Unable to parse all Company Codes.Please contact Henry Diep (x7209) for assistance.");'
  ...
  onclick("Compile", {
    ...
    for(i in listPolicies){
      ...
      listSupport <- list.files(paste(base,"Requests/append",i,"supporting",sep = "/"), pattern = 'pdf$')
        ...
        test<-(paste("WARNING: No supporting documents found for policy ",i,"."))
        js_string<-'alert("ALERT");'
        js_string<-sub("ALERT",test,js_string)
        session$sendCustomMessage(type='jsCode',list(value=js_string))
        print(paste("WARNING: No supporting documents found for policy ",i,sep = ""))
        }
...
ui<-fluidPage(
  ...
  headerPanel("Reinsurance Claims: Request for Payments"),
